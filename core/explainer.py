...
class AIExplainer:
    ...
    def _build_structured_explanation(self, payload: Dict[str, Any]) -> str:
        ...
        if doctors:
            lines.append("")
            lines.append("Suggested doctors based on the selected city and specialty include:")
            for doc in doctors[:3]:
                lines.append(
                    f"- {doc.get('name', 'Unknown')} in {doc.get('city', 'Unknown')} "
                    f"({doc.get('specialty', 'Unknown')}, rating {doc.get('rating', 'N/A')})"
                )
...
"""
    ...
    def _is_bad_rewrite(self, text: str) -> bool:
        ...
        # Common weak patterns from small/medium models
        bad_patterns = [
            "the doctor will",
